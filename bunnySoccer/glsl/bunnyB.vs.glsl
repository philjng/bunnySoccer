...
void main() {
    ...
    if (explodeB == 1) {
        gl_Position = projectionMatrix * viewMatrix * (modelMatrix * vec4(newPos, 1.0) + vec4(bunnyBPosition, 0.0));
    }
    else {
        gl_Position = projectionMatrix * viewMatrix * ( modelMatrix * vec4(position, 1.0) + vec4(bunnyBPosition, 0.0) );
    }
}